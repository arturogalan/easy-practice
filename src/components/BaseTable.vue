<script>
  ...
  export default {
    ...
    computed: {
      ...
      pagesNumber() {
        return Math.min(Math.ceil(this.rows.length / this.selectedPageSize), maxPagesNumber)
      },
      startRowNumber() {
...
<template>
  ...
  <div v-else>
    ...
    >
      <div
        class="grid grid-cols-4 gap-4 text-left relative"
      >
        <div class="absolute before:content-[''] before:block w-3 h-full rounded-l-md" :class="statusColorMapper(row)"/>
        <div
    ...
    <div class="w-auto mx-8 my-2 flex justify-between">
      ...
      >
        ...
        <div 
          ...
          :class="page === currentPage ? 'bg-green-800 text-white' : 'bg-white hover:bg-gray-50 text-black cursor-pointer'"
      ...
      >
        <option 
          ...
          :key="`option-${pageSize}`" 
          :value="pageSize">{{$t('baseComponents.baseTable.selectPagintacionSize', {pageSize, concept: conceptWord})}}
        </option>
      </select>
      </section>
      <div>{{ $t('baseComponents.baseTable.showPagination', {startRowNumber: startRowNumber + 1 , endRowNumber: rows.length < pageSize ? rows.length : endRowNumber, totalRowsNumber: rows.length, concept: conceptWord})}}</div>
    </div>
    </div>
</template>
